...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div class="h4 mb-0 text-primary">
            <i class="bi-palette-fill text-primary me-1"></i>
            @lang('app.attributes')
        </div>
        <div>
        ...
        <table class="table table-hover table-striped">
            ...
            @foreach($objs->sortBy('sort_order')->sortBy('parent.sort_order') as $obj)
                <tr>
                                ...
                                <table class="table table-hover table-striped">
                                    ...
                                    @forelse($obj->values as $value)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('admin.attributeValues.edit', $value->id) }}"
                                                   class="btn btn-success btn-sm my-1">
                                                    <i class="bi-pencil"></i>
                                                </a>
                                                <button type="button" class="btn btn-secondary btn-sm my-1"
                                                        data-bs-toggle="modal"
                                                        data-bs-target="#deleteV{{ $value->id }}">
                                                    <i class="bi-trash"></i>
                                                </button>
                                                <div class="modal fade" id="deleteV{{ $value->id }}" tabindex="-1"
                                                     aria-labelledby="deleteV{{ $value->id }}Label" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <div class="modal-title fs-5 fw-semibold"
                                                                     id="deleteV{{ $value->id }}Label">
                                                                    {{ $value->getName() }}
                                                                </div>
                                                            ...
                                                            <div class="modal-footer">
                                                                <form action="{{ route('admin.attributeValues.destroy', $value->id) }}"
                                                                      method="post">
                                                                    @method('DELETE')
                                                                    @csrf
                                                                    @honeypot
                                                                    ...
                                                                    <button type="submit"
                                                                            class="btn btn-secondary btn-sm"><i
                                                                                class="bi-trash"></i> @lang('app.delete')
                                                                    </button>
                                                                </form>
                                                            </div>
                                                        </div>
                    ...
                    <td>
                        <button class="btn btn-light btn-sm my-1" type="button" data-bs-toggle="collapse"
                                data-bs-target="#collapse{{ $obj->id }}" aria-expanded="false"
